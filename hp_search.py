        ...
        for k in [20, 50]:
            for w_hidden_sizes in reversed([[k] * 2]):
                for theta_hidden_sizes in reversed([[k] * 3]):
                    for w_lr in reversed([3e-7, 1e-7, 3e-8]):
                        for theta_lr in reversed([3e-8, 1e-8, 3e-9]):
                            MDP = get_Mountain_Car_MDP(n_bins=70)
                            agent = MLP_Agent(
                                MDP=MDP,
                                state_size=2,
                            ...
                            #parameters.append([MDP, agent, Baseline_Algorithm, n, save_dir])
                            pool.submit(func, MDP, agent, Baseline_Algorithm, n, save_dir)
